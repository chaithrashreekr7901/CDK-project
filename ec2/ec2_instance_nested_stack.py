# CRMP-PROJECT/cdk_project/ec2/ec2_instance_nested_stack.py
import logging
...
class Ec2InstanceNestedStack(NestedStack):
    # ... (keep existing attributes like cfn_ec2_instance, etc.)

    # --- MODIFICATION POINT 1: _generate_mount_script_block ---
    # It will now take `attachment_device_name` (e.g. /dev/sdf)
    # The `mount_config` will still provide mount_point, fs_type, owner, etc.
    # The `device_name_in_os` from mount_config will be ignored by this function.
    def _generate_mount_script_block(self, attachment_device_name: str, mount_config: dict) -> str:
        ...
        # attachment_device_name is something like /dev/sdf, /dev/sdg
        # mount_point is the target directory like /var/www/html/appdata
        ...
        if not attachment_device_name.startswith("/dev/"):
            ...
            # Could be an error, but let script try
        ...
        script_lines = [
            ...
            f"FS_TO_USE=\"{fs_type}\"",
            "OS_DEVICE_PATH=\"\"", # Will be populated after discovery
            ...
            "fi",
            "",
            "if [ -z \"$OS_DEVICE_PATH\" ]; then", # Should not happen if fallback works
            ...
            f"    echo \"Warning: Existing filesystem $EXISTING_FS_TYPE on $OS_DEVICE_PATH differs from configured $FS_TO_USE. Using existing.\" >&2",
            "  fi",
            "fi",
            "",
            ...
            "  if [ -n \"$UUID\" ]; then",
            f"    FSTAB_ENTRY=\"UUID=$UUID $MOUNT_TARGET $FS_TO_USE defaults,nofail,discard 0 2\"", # Added discard for SSDs
        ...
        elif owner_user: # Only user specified
        ...
        full_script.append("  return 0 # Explicit success") # Return 0 on success from function
        full_script.append("}")
        full_script.append(f"{function_name} || echo \"Function {function_name} reported an error.\" >&2") # Call the function and log if it returned non-zero

        return "\n".join(full_script)

    # --- MODIFICATION POINT 2: _resolve_user_data_with_mounts ---
    def _resolve_user_data_with_mounts(self) -> typing.Optional[str]:
        ud_conf = self.config.get("user_data", {})
        base_commands_list = []
        
        user_data_header = [
            "#!/bin/bash", # Corrected shebang
            "exec > >(tee /var/log/user-data.log|logger -t user-data -s 2>/dev/console) 2>&1",
            "set -x", 
            "echo \"Starting UserData script execution... $(date)\"", # Added date here too
        ]
        ...
        # Get enabled additional EBS devices and enabled mount configurations
        ...
        if configured_ebs_block_devices and configured_mount_configs:
            ...
            # Correlate by order: Nth mount config applies to Nth additional EBS volume
            ...
            for i in range(num_volumes_to_mount):
                ...
                attachment_name = ebs_device_info.get("device_name") # e.g., /dev/sdf
        ...
        if mount_script_blocks_list: # Check if list has more than just the initial comment
    ...
    #
    # ... PASTE ALL YOUR OTHER HELPER METHODS (_resolve_or_create_security_groups, etc.) HERE ...
    ...
    def _create_new_security_group(self, vpc: ec2.IVpc, sg_def: dict) -> typing.Optional[typing.List[str]]:
        ...
        try:
            ...
            logger.info(f"Created new Security Group '{sg_name}' (ID Token: {self.created_security_group_object.security_group_id})") # Log token for L2 SG
    ...
    def _resolve_iam_instance_profile_name(self, instance_name_tag_value:str) -> str | None: 
        ...
        if profile_conf.get("create_new", True): 
            ...
            instance_role = iam.Role(self, role_cdk_id, 
                                     assumed_by=iam.ServicePrincipal("ec2.amazonaws.com"), 
                                     role_name=role_name_cfg, 
                                     description=f"IAM Role for {instance_name_tag_value}")
            
            if self.config.get("ssm_session_permissions", False): 
            ...
            for i, arn in enumerate(profile_conf.get("managed_policy_arns", [])): 
                if not arn or not isinstance(arn, str): 
                    ...
                    continue
                if "AmazonSSMManagedInstanceCore" in arn and self.config.get("ssm_session_permissions", False):
                ...
                instance_role.add_managed_policy(iam.ManagedPolicy.from_managed_policy_arn(self, unique_mp_id, arn))
            ...
            if custom_policies and isinstance(custom_policies, list):
                ...
                if not policy_doc.is_empty: 
                    instance_role.attach_inline_policy(iam.Policy(self, f"{role_cdk_id}CustomPolicy", document=policy_doc))
            
            cfn_profile = iam.CfnInstanceProfile(self, profile_cdk_id, 
                                                 roles=[instance_role.role_name], 
                                                 instance_profile_name=profile_name_cfg)
            logger.info(f"Created new IAM Role '{instance_role.role_name}' and Instance Profile '{cfn_profile.instance_profile_name}'. Profile Ref: {cfn_profile.ref}")
            return cfn_profile.ref 
    ...
    # __init__ and other top-level methods for the class (unchanged from your previous version)
    def __init__(self, scope: Construct, construct_id: str,
        ...
        cfn_instance_props: dict = {
            ...
            "iam_instance_profile": iam_instance_profile_name_or_arn,
            ...
            # source_dest_check is per-ENI if multiple ENIs are defined, otherwise applies to primary
