    ...
    for k, v in E.items():
        if isinstance(v, dict) and k in result and isinstance(result[k], dict):
            result[k] = merge_dicts(result[k], v)
             ...
             # Simple list merge/replace - extend might be better depending on use case
             # result[k].extend(v) # Append
             result[k] = v # Replace
        else:
            result[k] = v
    return result
    ...
    def __init__(self, scope: Construct, id: str, s3_deployments_config: dict, **kwargs) -> None:
        ...
        for i, bucket_cfg_entry in enumerate(bucket_configurations_list):
            ...
            for item_index in range(count):
                ...
                # --- Merge Configs: Defaults <- Bucket Specific ---
                merged_config = merge_dicts(default_bucket_config, bucket_specific_config) 

                # --- Generate Unique CDK ID for the Nested Stack Instance ---
                sanitized_config_id_base = ''.join(filter(str.isalnum, config_id_base))
                if not sanitized_config_id_base: sanitized_config_id_base = f"Bucket{i}"
                
                instance_suffix = f"{item_index + 1}" if count > 1 else "" # Add index only if count > 1
                # Ensure final ID is valid
                ...
                # Add instance index to config if needed by instance stack (e.g., for naming)
                merged_config['_instance_index_'] = item_index # Use internal key
                ...
                try:
                    bucket_stack = S3BucketInstanceStack(
                        self,
                        nested_stack_cdk_id,
                        s3_merged_config=merged_config, # Pass the final merged config
                        description=f"Nested Stack for S3 Bucket: {bucket_name_for_desc}"
                    )
                    # Store reference using a unique key combining base ID and index
                    ...
                    # Decide behavior: continue or raise? Continuing allows other buckets to deploy.
                    # raise e # Uncomment to stop deployment on first bucket failure
